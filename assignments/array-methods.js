...
runners.forEach(runner => {
    if (runner.donation < 100) {
        cheapSkates.push({ offender: `${runner.first_name} ${runner.last_name}`, offensive_amount: runner.donation, way_to_track_down: runner.email });
    }
});
...
// Problem 2
//  Because the director is too concerned with name case, and not the finances, last years cheap skates were not tracked and properly harassed.
//  This caused a budget shortage. Due to this budget shortage, we will only be able to afford one shirt of each size. In order to be equitable, 
//  everyone will share the shirt in their size group. Break the runners into their size groups, and prioritize sharing order by donation size.
//  Cheap skates get the sweaty shirt.

function shirtSwap(size, cb) {
    return cb(runners.filter(runner => runner.shirt_size === size).sort((a, b) => { return a.donation - b.donation }).reverse());
}
